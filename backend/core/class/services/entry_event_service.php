...
class EntryEventService implements EntryEventInterface{
	...
	public function makeDataAsNewTable($key, $fileName, $data) {
		...
		if(true){
			...
			$newData 	= $fileHelper->createNewExcelFileUsingNewColumnsFormat("1", $fileName,$data) ;
			...
			if (!empty($data)) {
				//\R::begin();
					...
					foreach ($data as $row) {
						...
						$ifRefProductExist 		= \R::getRow("select id,range_id from products where reference = :ref",
															array(':ref'=>$row['Référence']));
