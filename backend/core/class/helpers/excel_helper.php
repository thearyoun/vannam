        ...
        try {
            ...
            foreach ($data as $row) {
                ...
                foreach ($row[0] as $column) {
                    if ($isMultipleColumns) {
                        $currentRangeInUtile = R::getRow("select * from rangedetails where name = :range", array(':range' => $column['value']));
                        ...
                        array_push($tabNamesOfRanges, array(
                                "old_name" => $nameNewColumn,
                    ...
                    $AllcolumnsData = $sheet->rangeToArray("A1:$highestColumn{$wsD[0]['totalRows']}", NULL, TRUE, FALSE);
                    ...
                    foreach ($AllcolumnsData as $nRow) {
                        ...
                        $objPHPExcel->getActiveSheet()->fromArray($tabArrayValue, NULL, "{$highestColumn}{$i}");
                        $i++;
                    }
                }
            }
            ...
            $AllNewData = $sheetSecond->rangeToArray("A1:{$wsDS[0]['lastColumnLetter']}{$wsDS[0]['totalRows']}", NULL, TRUE, FALSE);
            $newTableData = [];
            ...
            return json_encode($newTableData);

        } catch (PHPExcel_Writer_Exception $e) {
            $logger->debug('Error loading file "' . pathinfo($name, PATHINFO_BASENAME) . '": ' . $e->getMessage());
            return $e->getMessage();
        }
    }
